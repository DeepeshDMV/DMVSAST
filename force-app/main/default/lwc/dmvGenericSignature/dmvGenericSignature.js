...
let currPos = { x: 0, y: 0 };
let prePos = { x: 0, y: 0 };
export default class SignaturePanel extends OmniscriptBaseMixin(
  LightningElement
) {
  @api recordId;
  ...
  handleSaveSignature(evt) {
    ...
    let imageURL = sCanvas.toDataURL("image/jpeg");
    let imageData = imageURL.replace(/^data:image\/(png|jpg);base64,/, "");
    console.log(imageData);
    this.omniSaveState(imageData);
    this.OmniApplyCallResp({ Output: imageData });
