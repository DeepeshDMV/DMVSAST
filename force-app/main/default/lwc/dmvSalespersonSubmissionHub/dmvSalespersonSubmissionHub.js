...
export default class DmvSalespersonSubmissionHub extends OmniscriptBaseMixin(
  ...
  //     this.omniRemoteCall(params, false)
  //       .then((response) => {
  //         window.console.log("submission hub status", response);
  //       })
  //       .catch((error) => {
  //         // window.console.log(error, "error");
  //         // this.showData = true;
  ...
  handleButtonClick(data) {
    try {
      console.log(
        "*step inside handleButtonClick DMV Submission hub",
        data.paymentUrl
      );
      ...
      if (this.isSalesHub != "true") {
        ...
        this.omniRemoteCall(params, false)
          .then((response) => {
            ...
            window.console.log(
              "Response from Business Hub startFlow",
              response
            );
          })
          .catch((error) => {
  ...
  startFlow() {
    ...
    this.downloadUrl =
      "https://" +
      new URL(mitCaseUrl).hostname +
      "/apex/SPLicenseDownload?id=" +
      this.accountId;
    //  }
    // else {
    //   const params = {
  ...
  handleHearingRequest(event) {
    ...
    this.omniRemoteCall(params, false)
      .then((data) => {
        console.log("DataFiles line 45" + JSON.stringify(data.result.success));
          ...
          if (data.result.success === "Error Date Exceeded 60 days") {
            this.dispatchEvent(
              new ShowToastEvent({
                title: "Error!",
                message:
                  "Your application was denied more than 60 days ago. You are no longer eligible to submit hearing requests.",
                variant: "error"
              })
            );
          } else if (
            data.result.success === "Hearing Request already submitted"
          ) {
            this.dispatchEvent(
              new ShowToastEvent({
                title: "Error!",
